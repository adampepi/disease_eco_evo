...
###Function to simulation population dynamics, with multiple runs to see allele frequency change
ecoevo_sims<-function(model=outbreak_model,vs=c(0.75,0.5,0.25),rs=c(15,10,5),mu=0.4,tau=12, V=0.86^2,X0=0.01, P0=0.1, N0=0.5,Tmax=100,Years=10,phi=3,nsims=10, popfactor=1000){
  ...
  xmat<-matrix(data=NA,nrow=Years,ncol=nsims) ##Matrix of allele frequency trajectories
  for(j in 1:nsims){
...
s1<-ecoevo_sims(Years=200,vs=c(0.7,0.67, 0.64),rs=c(20,18,16), phi=3,X0=1/1000,P0=1, N0=2.5,nsims=100,popfactor=1000)
s1

matplot(s1,type='l')
evo1<-as.data.frame(s1)
evo1$t<-seq(1:200)
str(evo1)
evo2<-melt(evo1,id.vars='t')
str(evo2)

evo2
ggplot(evo2,aes(x=t,y=value,color=variable))+geom_line()+theme_classic()+scale_color_viridis_d()+ylab('Disease resistance allelle frequency')+theme(legend.position = 'none')
